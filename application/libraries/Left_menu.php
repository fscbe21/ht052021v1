...
class Left_menu {
    ...
    private function _get_sidebar_menu_items($type = "") {
        ...
        if ($this->ci->login_user->user_type == "staff" && $type !== "client_default") {
            ...
            $access_products = get_array_value($permissions, "access_products");
            $add_purchase = get_array_value($permissions, "add_purchase");
            $purchase_list = get_array_value($permissions, "purchase_list");

            $grn_list = get_array_value($permissions, "grn_list");
            $purchase_requisition = get_array_value($permissions, "purchase_requisition");
            $purchase_quotation = get_array_value($permissions, "purchase_quotation");

            $purchase_order_list = get_array_value($permissions, "purchase_order_list");

            
            
            
            $add_purchase_order = get_array_value($permissions, "add_purchase_order");

            $add_sales = get_array_value($permissions, "add_sales");

            $sales_list = get_array_value($permissions, "sales_list");

            $add_transfer = get_array_value($permissions, "add_transfer");

            $transfer_list = get_array_value($permissions, "transfer_list");

            /* AG2103Q */
            $view_production = get_array_value($permissions, "view_production");

            $work_order_list = get_array_value($permissions, "work_order_list");

            $bom_list = get_array_value($permissions, "bom_list");

            $lead_approval = get_array_value($permissions, "lead_approval");

            $custom_access_lead =  get_array_value($permissions, "lead");

            $warehouse_permission =  get_array_value($permissions, "warehouse_permission");


            $account_permission =  get_array_value($permissions, "account_permission");
            $payments_permission =  get_array_value($permissions, "payments_permission");
            $petrol_permission =  get_array_value($permissions, "petrol_permission");
            $expense_approval =  get_array_value($permissions, "expense_approval");
            $dc_outward =  get_array_value($permissions, "dc_outward");

            $hrm_permission =  get_array_value($permissions, "hrm_permission");

            $add_sales_return =get_array_value($permissions, "add_sales_return");
            $sales_return_list =get_array_value($permissions, "sales_return_list");
            $add_purchase_return =get_array_value($permissions, "add_purchase_return");
            $purchase_return_list =get_array_value($permissions, "purchase_return_list");
            $sales_order =get_array_value($permissions, "sales_order");
            $sales_quotation =get_array_value($permissions, "sales_quotation");

            //end new list
            $quality_check =  get_array_value($permissions, "quality_check");
            ...
            if (get_setting("module_message") == "1" && $access_messages) {
                $sidebar_menu["messages"] = array("name" => "messages", "url" => "messages", "class" => "fa-envelope", "badge" => count_unread_message(), "badge_class" => "badge-secondary");
            }

            if (get_setting("module_message") == "1" && $access_messages) {//darini 18-2
               
                $sidebar_menu["mom"] = array("name" => 'mom', "url" => "mom", "class" => "fa fa-users","devider" => true, "badge" => count_unread_message1(), "badge_class" => "badge-secondary");
...
$crm_list_array=array();



if($this->ci->login_user->is_admin ||$access_client  ){
array_push($crm_list_array,array("name" => "clients", "url" => "clients", "class" => "fa-briefcase"));
}


if($this->ci->login_user->is_admin ||$custom_access_lead){
    array_push($crm_list_array,array("name" => "leads", "url" => "leads"));
            }


            if($this->ci->login_user->is_admin ||$client_approval){
                array_push($crm_list_array,array("name" => "client_approval", "url" => "client_approval", "class" => "fa fa-thumbs-up"));
            }



            if($this->ci->login_user->is_admin ||$lead_approval){
                array_push($crm_list_array,array("name" => "leaddataapproval", "url" => "lead_approval", "class" => "fa-wrench"));
            }
                ...
                if ($this->ci->login_user->is_admin || $access_client) {
                    ...
                    $sidebar_menu["clients"] = array("name" => "crm", "url" => "clients", "class" => "fa-briefcase",
                    "submenu" => $crm_list_array
                );
...
$purchase_array_list = array();

array_push($purchase_array_list,array("name" => "product_list", "url" => "products"));



if($purchase_quotation || $this->ci->login_user->is_admin ){
    /* array_push($purchase_array_list,array("name" => "quotations", "url" => "quotations", "class" => "fa-file-text-o")
); */
    }

//R.V29_03_S
   // if($purchase_quotation || $this->ci->login_user->is_admin ){
    /* array_push($purchase_array_list,  array("name" => "indent", "url" => "indent", "class" => "fa-file-text-o")); */ /** AG2403 */
//}//R.V29_03_E


if($purchase_list || $this->ci->login_user->is_admin ){
array_push($purchase_array_list,array("name" => "purchase_list", "url" => "purchase"));
}
...
if($add_purchase || $this->ci->login_user->is_admin){
    array_push($purchase_array_list,array("name" => "add_purchase", "url" => "purchase/create", "class" => "fa-file-text-o"));
    }




if($purchase_order_list || $this->ci->login_user->is_admin){
                        array_push($purchase_array_list,array("name" => "purchase_order_list", "url" => "purchase_order", "class" => "fa-file-text-o"));
                        }


if($add_purchase_order || $this->ci->login_user->is_admin){
                            array_push($purchase_array_list, array("name" => "add_purchase_order", "url" => "purchase_order/create", "class" => "fa-file-text-o"));
                            }



                            if($add_purchase_return || $this->ci->login_user->is_admin ){
                                array_push($purchase_array_list,array("name" => "purchase_return", "url" => "purchase_return"));
                                }
        ...
        if(count($purchase_array_list) || $this->ci->login_user->is_admin){

            $sidebar_menu["purchase"] = array("name" => "purchase", "url" => "noc", "class" => "fa fa-shopping-cart",
            "submenu" => $purchase_array_list
...
$sale_array_list = array();

if($add_sales || $this->ci->login_user->is_admin ){
array_push($sale_array_list,   array("name" => "add_sales", "url" => "sales/add_sales", "class" => "fa-file-text-o"));
}


if($sales_list || $this->ci->login_user->is_admin ){
    array_push($sale_array_list,  array("name" => "sales_list", "url" => "sales"));
    }



    if($add_sales_return || $this->ci->login_user->is_admin ){
        array_push($sale_array_list,  array("name" => "sales_return", "url" => "sale_return"));
        }
        //darini 24-3
        if($this->ci->login_user->is_admin ){
            array_push($sale_array_list,  array("name" => "sales_order", "url" => "sales_order"));//changes 24-3
            array_push($sale_array_list,  array("name" => "sales_quatation", "url" => "sales_qutation"));//changes 24-3
        }
    

if(count($sale_array_list) ||  $this->ci->login_user->is_admin){

      //darini 11-3s
      /* $sidebar_menu["sales"] = array("name" => "sales", "url" => "#", "class" => "fa fa-balance-scale",
      "submenu" =>$sale_array_list
      ); */
        //darini 19-3
...
$transfer_array_list = array();

if($warehouse_permission || $this->ci->login_user->is_admin){

    array_push($transfer_array_list,array("name" => "warehouse", "url" => "warehouse", "class" => "fa-file-text-o"));

    array_push($transfer_array_list,array("name" => "overall_stock_report", "url" => "report/overall_stock_report", "class" => "fa-file-text-o"));
    
}


if($grn_list || $this->ci->login_user->is_admin){
    array_push($transfer_array_list, array("name" => "grn", "url" => "grn"));
    }


if($purchase_requisition || $this->ci->login_user->is_admin){
            array_push($transfer_array_list, array("name" => "purchase_requisition", "url" => "purchase_request/create"));
            }


if($add_transfer || $this->ci->login_user->is_admin ){
array_push($transfer_array_list, array("name" => "add_transfer", "url" => "transfer/add_transfer", "class" => "fa-file-text-o"));
}


if($transfer_list || $this->ci->login_user->is_admin ){
    array_push($transfer_array_list,array("name" => "transfer_list", "url" => "transfer"));
    }

    array_push($transfer_array_list,  array("name" => "indent", "url" => "indent", "class" => "fa-file-text-o")); // AG16042021

   

    if( $this->ci->login_user->is_admin || $dc_outward){
        array_push($transfer_array_list, array("name" => "dc_outward_list", "url" => "production_dc_outward"));
    }

    if( $this->ci->login_user->is_admin || $dc_outward ){
        array_push($transfer_array_list, array("name" => "dc_outward_add", "url" => "production_dc_outward/create"));
    }

    array_push($transfer_array_list,array("name" => "warehouse_stock_report", "url" => "report/warehouse_stock_report"));

    if(count($warehouse_permission) || $this->ci->login_user->is_admin){

        $sidebar_menu["warehouse"] = array("name" => "warehouse", "url" => "#",  "class" => "fa fa-home", "submenu" =>$transfer_array_list
    );
}

/*AG041021 START*/

$production_array_list=array();
/* AG2103Q */
if($view_production || $this->ci->login_user->is_admin ){
    array_push($production_array_list,  array("name" => "view_production", "url" => "viewproduction", "class" => "fa-file-text-o"));
    
    }

if($work_order_list || $this->ci->login_user->is_admin ){
    array_push($production_array_list,  array("name" => "work_order_list", "url" => "work_order/index", "class" => "fa-file-text-o"));
    }
    
    
    if($bom_list || $this->ci->login_user->is_admin ){
        array_push($production_array_list, array("name" => "bom_list", "url" => "bomcreation"));
        }

        //start dsk 21march2021     
    
            array_push($production_array_list, array("name" => "production_stock", "url" => "report/production_stock_report")); //AG2603

            //end dsk 21march2021
        if($this->ci->login_user->is_admin || $quality_check){
            array_push($production_array_list, array("name" => "qccheck", "url" => "qcpass"));
        }
             
            array_push($production_array_list, array("name" => "reuse_list", "url" => "reuse")); 
            
            array_push($production_array_list,  array("name" => "indent", "url" => "indent", "class" => "fa-file-text-o")); /** AG2403 */


        if(count($production_array_list) ||  $this->ci->login_user->is_admin){
            $sidebar_menu["production"] = array("name" => "production",  "url" => "noc", "class" => "fa fa-cogs",
            "submenu" => $production_array_list
            );
        }
...
if($this->ci->login_user->is_admin ||$account_permission){
array_push($account_list_array, array("name" => "account_list", "url" => "accounting"),
array("name" => "money_tranfer", "url" => "accounting/money_transfer", "class" => "fa-file-text-o"),
array("name" => "bank_summary", "url" => "accounting/balance_sheet", "class" => "fa-file-o"),
array("name" => "loan", "url" => "loan", "class" => "fa-file-o"),
array("name" => "account_statement", "url"=>"accounting/statement_model", "class" => "fa-file-o"));
}

if($this->ci->login_user->is_admin || $sales_order ){
    array_push($account_list_array,  array("name" => "sales_order", "url" => "sales_order"));//changes 30-4
   
}
if($this->ci->login_user->is_admin || $sales_quotation){
        
    array_push($account_list_array,  array("name" => "sales_quatation", "url" => "sales_qutation"));//changes 30-4
}

/* if($this->ci->login_user->is_admin ||$access_estimate){
array_push($account_list_array,array("name" => "estimate_list", "url" => "estimates"),
...
if($this->ci->login_user->is_admin ||$access_invoice){

    array_push($account_list_array,array("name" => "invoices", "url" => "invoices", "class" => "fa-file-text"));

}



if($this->ci->login_user->is_admin ||$payments_permission){

    array_push($account_list_array, array("name" => "invoice_payments", "url" => "invoice_payments", "class" => "fa-money"),
    array("name" => "payments_due", "url" => "clients/payment_due", "class" => "fa-money"));

}


if($this->ci->login_user->is_admin ||$access_expense){
    array_push($account_list_array,array("name" => "expenses", "url" => "expenses", "class" => "fa-money"));
}


if($this->ci->login_user->is_admin || $petrol_permission){
    array_push($account_list_array,array("name" => "petrol_allowance", "url" => "expenses/petrol_allowance", "class" => "fa-money"));
}


if($this->ci->login_user->is_admin || $expense_approval){
    array_push($account_list_array, array("name" => "expenses_approval", "url" => "expenses_approval", "class" => "fa-money"));
}


if($this->ci->login_user->is_admin){
    array_push($account_list_array,  array("name" => "income_vs_expenses", "url" => "expenses/income_vs_expenses", "class" => "fa-line-chart"));
}



if(count($account_list_array) || $this->ci->login_user->is_admin){
    $sidebar_menu["accounting"] = array("name" => "accounting", "url" => "#", "class" => "fa fa-briefcase",
    "submenu" => $account_list_array
    );//end
}
...
if($this->ci->login_user->is_admin || $hrm_permission){

    $sidebar_menu["hrm"] = array("name" => "hrm", "url" => "#", "class" => "fa-user",
    "submenu" => array(
    
        array("name" => "employee_mgmt", "url" => "team_members", "class" => "fa-user font-16"),
...
//darini19-2
 ...
 //darini 20-3
//  $sidebar_menu["return"] = array("name" => "return", "url" => "#", "class" => "fa fa-undo",
//  "submenu" => array(
//      array("name" => "sales_return", "url" => "sale_return"),
//      array("name" => "purchase_return", "url" => "purchase_return"),
 ...
 */
            $sidebar_menu["logistics"] = array("name" => "logistics", "url" => "logistics", "class" => "fas fa-truck",
            "submenu" => array(
                array("name" => "employee_mgmt", "url" => "logistics_members", "class" => "fa-user font-16"),//R.V 23_03
                 array("name" => "vehicle", "url" => "logistics") 
                /* array("name" => "pickup", "url" => "pickup", "class" => "fa-file-text-o"),
                array("name" => "tracking", "url" => "tracking"),
                array("name" => "services", "url" => "services", "class" => "fa-file-text-o")*/
            )
            );

            if($this->ci->login_user->is_admin){

                $sidebar_menu["reports"] = array("name" => "reports", "url" => "#", "class" => "fa-line-chart", "devider" => true, 
                "submenu" => array(
                
                    array("name" => "purchase_report", "url" => "reports", "class" => "fa-file-o"),
                    /* array("name" => "payments_due_clients", "url" => "report_due", "class" => "fa-file-o"), */
                    array("name" => "payment_report", "url" => "report_payment", "class" => "fa-file-o"),
                    array("name" => "product_report", "url" => "report_product", "class" => "fa-file-o"),
                    array("name" => "vehicle_insurance", "url" => "report/vehicleInsuranceReport", "class" => "fa-file-o"),
                    array("name" => "transfer_report", "url" => "report/transfer_report", "class" => "fa-file-o"),//R.V22_04
                    array("name" => "overall_stock_report", "url" => "report/overall_stock_report", "class" => "fa-file-o")
                )
                );
            }
            ...
            if (get_setting("module_leave") == "1" && ($this->ci->login_user->is_admin || $access_leave)) {
                $sidebar_menu["leaves"] = array("name" => "leaves", "url" => "leaves", "class" => "fa-sign-out font-16");
            } else if (get_setting("module_leave") == "1") {
                $sidebar_menu["leaves"] = array("name" => "leaves", "url" => "leaves/leave_info", "class" => "fa-sign-out font-16");
            }
    ...
    */
    ...
    //R.V End23_02
            ...
            if ($this->ci->login_user->is_admin) {
                $sidebar_menu["settings"] = array("name" => "settings", "devider" => true, "url" => "settings/general", "class" => "fa-wrench");
            }
            //darini 12-2
