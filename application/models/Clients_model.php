...
class Clients_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if(($clientapproval != '') && (! $this->login_user->is_admin))
        {
            if($clientapproval == 1){
                $last_approved = 2;
    ...
    //start 3feb2021 dsk
    function company_name_details($company_name) {

        $clients_table = $this->db->dbprefix('clients');     

        $sql = "SELECT $clients_table.id, $clients_table.assigned_to 
        FROM $clients_table  
        WHERE $clients_table.deleted=0  AND REPLACE($clients_table.company_name, ' ', '') = '$company_name'";
    ...
    function count_total_clients($show_own_clients_only_user_id = "") { //anand
        $clients_table = $this->db->dbprefix('clients');

        $show_own = $this->login_user->id;
        $where = "";
        ...
        if ($show_own) {
            $where .= " AND $clients_table.owner_id=$show_own";
        }        
    ...
    function count_total_leads($show_own_leads_only_user_id = "") {
        $clients_table = $this->db->dbprefix('clients');
        $show_own_leads_only_user_id = $this->login_user->id;
        $where = "";

        if($this->login_user->is_admin)
        {
            $show_own_leads_only_user_id = 0;
        }else if($this->login_user->role_id != 3){
            $show_own_leads_only_user_id = 0;
        }
    ...
    function count_open_lead($show_own = "", $type = ""){
        ...
        $sql = "SELECT * FROM $clients_table  
                WHERE $clients_table.deleted=0 AND is_lead=1 AND lead_status_id 
                IN $in $where";     
    ...
    //darini 9-2
    function updateclientowner($owner_id,$id){
        $clients_table = $this->db->dbprefix('clients');
        $sql = "UPDATE $clients_table SET $clients_table.owner_id = $owner_id  WHERE $clients_table.id=$id";
        return $this->db->query($sql);
    }
    //end
    ...
    /* 
        AG12022021
        get approval pending list for currently viewing person's role
        input - current employee role
        output array - company name
        created on 12 Feb 2021 
    */
    function current_approval_pending_list($current_role = 0){
        $sql = "SELECT company_name FROM clients WHERE last_approved = $current_role+1";
    ...
    /* 
        AG12022021
        get latest client name list for currently viewing person
        input - current employee id
        output array - company name
        created on 12 Feb 2021 
    */
    function get_latest_client_list($user_id = 0){
        $sql = "SELECT company_name FROM clients WHERE owner_id=$user_id ORDER BY id DESC LIMIT 5";
    ...
    function materials_info($user_id = 0){
        $sql = "SELECT SUM(quantity) as qty, unit, product  FROM clients WHERE owner_id=$user_id AND is_lead=0 group by product";
      ...
      //start 12feb2021 dsk
      function company_branch_details($branch_details,$company_name) {

        $clients_table = $this->db->dbprefix('clients');     

        $sql = "SELECT $clients_table.id, $clients_table.assigned_to 
        FROM $clients_table  
        WHERE $clients_table.deleted=0  AND REPLACE($clients_table.company_name, ' ', '') = '$company_name'  AND REPLACE($clients_table.branch, ' ', '') = '$branch_details' ";

        return $this->db->query($sql);
    }

    function company_unit_details($unit,$branch,$company_name) {

        $clients_table = $this->db->dbprefix('clients');     

        $sql = "SELECT $clients_table.id, $clients_table.assigned_to 
        FROM $clients_table  
        WHERE $clients_table.deleted=0  AND REPLACE($clients_table.company_name, ' ', '') = '$company_name'  AND REPLACE($clients_table.branch, ' ', '') = '$branch'  AND REPLACE($clients_table.company_unit, ' ', '') = '$unit' ";
    ...
    function check_client($client_id, $owner_id){
        $sql = "SELECT * FROM clients WHERE id=$client_id AND owner_id=$owner_id";
        return $this->db->query($sql)->num_rows();
    }
    //darini 11-3
    function client_list(){
        $sql = "SELECT * FROM clients WHERE is_lead=0 ORDER BY company_name ASC";
        return $this->db->query($sql)->result();
    }//R.V_24_04 [Modiofied]
