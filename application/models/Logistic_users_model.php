...
class Logistic_users_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        $department_id = get_array_value($options, "department");
        ...
        $sql = "SELECT $users_table.*,
            ...
            $team_member_job_info_table.salary_term 
            $select_custom_fieds
        FROM $users_table
        LEFT JOIN $team_member_job_info_table ON $team_member_job_info_table.user_id=$users_table.id
        LEFT JOIN $department_table ON $department_table.id=$users_table.department
        LEFT JOIN $clients_table ON $clients_table.id=$users_table.client_id
        $join_custom_fieds    
        WHERE $users_table.deleted=0 AND $department_table.name='Logistics' OR $department_table.name='logistics' $where
        ORDER BY $users_table.first_name";
        return $this->db->query($sql);
        //$users_table.department=6
    }
    ...
    function team_members() {
        ...
        $sql = "SELECT *
                FROM $users_table
                WHERE deleted=0 AND $users_table.user_type='staff'
                ORDER BY $users_table.first_name";
    ...
    function active_member(){
        $sql  = "SELECT * FROM  users  WHERE status='active' AND  deleted=0 AND disable_login=0 ";
        return $this->db->query($sql)->result();
    }

    function userJobInfo($userid){
        $sql  = "SELECT * FROM team_member_job_info WHERE user_id = $userid";
        return $this->db->query($sql)->result();
    }
