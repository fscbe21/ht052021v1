...
class Attendance_model extends Crud_model {
    ...
    function log_time($user_id, $note = "",$department =0) {
    ...
    /* 
        AG12022021
        get current clock in users without view person
        input - current employee who views report
        output array - fullname
        created on 12 Feb 2021 
    */
    function members_currently_clocked_in($current_emp = 0){
        $sql = "SELECT user_id FROM attendance WHERE user_id != $current_emp AND status='incomplete' AND in_time >= date('Y-m-d')";
