...
class Mom_model extends Crud_model {
     ...
     */
    function get_details_test($options = array()) {
        $mom_table = $this->db->dbprefix('mom');
        $where = "";
        $id = get_array_value($options, "id");
        if ($id) {
            $where = " AND $mom_table.id=$id";
        }

        $where .= " ORDER BY id DESC";

        $sql = "SELECT $mom_table.*
        FROM $mom_table
        WHERE $mom_table.deleted=0 $where";
        return $this->db->query($sql);
    }

    function get_details($options = array()) {
        $messages_table = $this->db->dbprefix('mom');
