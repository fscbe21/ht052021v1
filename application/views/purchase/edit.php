    ...
    ?>
      ...
      <?php
        ...
        <div class="">
            <div class="panel panel-default">
                <div class="page-title clearfix">
                    ...
                    <a href="<?php echo_uri("purchase") ?>">
                            <button class="btn btn-md btn-default">Purchase List</button>
                    </a>
                    </div>
                </div>
                <br />
                <?php echo form_open(get_uri("purchase/updatepurchase"), array("id" => "purchase-form", "class" => "general-form", "role" => "form")); ?>
                <div class="container row">
                    <div class="col-md-5">
                        <div class="form-group">
                            <label for="title" class=" col-md-3">Warehouse *</label>
                            <input type="hidden" name="purchase_id" value="<?= $purchase_data->id; ?>"/>
                            <input type="hidden" name="warehouse_id_old" value="<?= $purchase_data->warehouse_id; ?>"/>
                ...
                <div class="container row">
                    <div class="col-md-5">
                        <div class="form-group">
                            <label for="title" class=" col-md-3">Purchase Status *</label>
                            <div class="<?php echo $field_column; ?>">
                                <div class=" col-md-9">
                                  <select name="status" class="form-control" required> 
                                  <?php
                                        $purchase_status = $purchase_data->status;
                                  ?>  
                                    <option value="" >Select Purchase Status</option>  
                                    <?php foreach($purchase_status_all as $psa){?>
                                            <option 
                                            
                                            <?php
                                        echo ($purchase_status == $psa->id) ? ' selected' : '';
                                            ?>
        ...
        <!--darini 10-4-->
                    ...
                    <div class="col-md-5">
                        <div class="form-group">
                            <label for="title" class=" col-md-3"> GRN Id</label>
                            <div class="<?php echo $field_column; ?>">
                                <div class=" col-md-9">
                                <select name="grn_id" class="form-control" disabled="true">   
                                    <option value="" >None</option>  
                                    <?php foreach($grn as $psa){?>
                                            <option value="<?php echo $psa->id; ?>"  <?php if( $purchase_data->grn_id == $psa->id){ ?> selected <?php  } ?>   ><?php echo $psa->id; ?> </option>
                                            <?php }  
                                            ?>                       
                                  </select> 
                                </div>
...
<!--end-->
                            ...
                            <div class="<?php echo $field_column; ?>">
                                ...
                                <select class="form-control" disabled="true">   
                                    <option value="">None</option> 
                                            ...
                                            <option value="1"  <?php if( $purchase_data->request_order == 1){ ?> selected <?php  } ?> >Purchase Requstion Number</option>
                                            <option value="2"  <?php if( $purchase_data->request_order == 2){ ?> selected <?php  } ?> >Purchase Order Number</option>
                   ...
                   <input type="hidden"  name="request_order" value="<?php echo $purchase_data->request_order ?>"/>
                   <input type="hidden" name="request_number" value="<?php echo $purchase_data->request_number ?>"/>
                   
                    <div class="col-md-5" id="o_number" <?php if($purchase_data->request_order==null || $purchase_data->request_order==0) {?> style="display:none" <?php }?> >
                        <div class="form-group">
                            <label for="title" class=" col-md-3" id="title_number"><?php if( $purchase_data->request_order == 1){ echo "Purchase Requstion Number";}else if($purchase_data->request_order == 2){ echo "Purchase Order Number";}?></label>
                            <div class="<?php echo $field_column; ?>">
                                <div class=" col-md-9">
                                <!--  <input type="number" name="purchase_requstion_number" class="form-control"  required>-->
                                <select  id="request_number" class="form-control" disabled="true">   
                                    
                                            <?php if( $purchase_data->request_order == 1){?>
                                                    <option value="">None</option> 
                                                    <?php foreach($request as $tx){?>
                                                        <option value="<?php echo $tx->id; ?>" <?php if( $purchase_data->request_number == $tx->id){ ?> selected <?php  } ?>  ><?php echo $tx->prno; ?> </option>
                                                      <?php } 

                                            }else if( $purchase_data->request_order == 2){ ?> 
                                                    <option value="">None</option> 
                                                    <?php foreach($order as $tx){?>
                                                        <option value="<?php echo $tx->id; ?>" <?php if( $purchase_data->request_number == $tx->id){ ?> selected <?php  } ?> ><?php echo $tx->po_no; ?> </option>
                                                      <?php } }?>
                ...
                <div class="table-responsive mt-3">
                    <table id="myTable" class="table table-hover table-striped order-list" style="width: 80%; font-weight: 600">
                        ...
                        <?php
                           $options = array();
                           $options['purchase_id'] = $purchase_data->id;
                            $purchase_detail = $this->Productpurchase_model->get_details($options)->result();
    ...
    $unit_data    = $this->Unit_model->get_one($pdata->purchase_unit_id);
    $unit_name    = $unit_data->name;
                        ...
                        <td><?= $unit_name; ?></td>
                        <td ><input type="text" class="cost" value="<?= $pdata->net_unit_cost; ?>"/></td>
                        <td><?= $pdata->discount; ?></td>
                        <td class="tax"><?= $pdata->tax; ?></td>
                        <td class="subtotal"><?= $pdata->total; ?></td>
                        <td style="color: red"><i class="fa fa-trash-o deletethis" aria-hidden="true"></i><input type="hidden" name="code[]" value="<?= $product_code; ?>"/>
                        <input type="hidden" name="id[]" value="<?= $pdata->product_id; ?>"/>
                        <input type="hidden" name="product_cost[]"  class="product_cost" value="<?= $pdata->net_unit_cost; ?>"/>
              ...
              <div class="container p-2">
                   <div class="row" style="width: 80%">
                            ...
                            <select name="order_tax" class="form-control">   
                                    <option value="">Select Order Tax</option> 
                                        <?php foreach($tax_all as $tx){?>
                                            <option value="<?php echo $tx->id; ?>" <?php if($purchase_data->order_tax== $tx->id){?> selected <?php } ?> ><?php echo $tx->title; ?> </option>
                        <?php }  ?>                                                    
                                </select>
                        </div>
                        ...
                        <div class="col-md-4">
                            <p>Discount</p>
                            <input type="number" name="total_discount" class="form-control" value="<?= $purchase_data->order_discount; ?>"/>
                        </div>

                        <div class="col-md-4">
                            <p>Shipping Cost</p>
                            <input type="number" name="shipping_cost" class="form-control" value="<?= $purchase_data->shipping_cost; ?>"/>
                        </div>                    
                   </div>
                   <br/>
                   <div class="row" >
...
<script type="text/javascript">
    ...
    $(document).on('click', '.selected', function(){
        ...
        if(indexv == 1){
            ...
            var cost = $('#order-table tr:eq(' + closestTr + ')').find('.cost').val();
        ...
        else
        {
            ...
            orderlist += '<td class="unit-name"></td>';//changes 23-3
    ...
    $(document).on('input', '.qtychange', function(){
        ...
        var cost = $('#order-table tr:eq(' + row_index + ')').find('.cost').val();
        cost = parseFloat(cost);
    ...
    $(document).on('input', '.cost', function(){
       ...
       // console.log("tax"+(parseFloat(cost * qty)*taxrate)/100);
        ...
        var subTotal = parseFloat(cost * qty)+parseFloat(tax)    ;   
        subtotal = subTotal.toFixed(2);
        $('#order-table tr:eq(' + row_index + ')').find('.product_cost').val(cost);
        $('#order-table tr:eq(' + row_index + ')').find('.subtotal').text(subtotal);
    });
...
$(document).on('click', '#fs-supplier-btn', function(){
    $.ajax({
            type: 'POST',
            dataType: 'json',
            url: '<?php echo base_url(); ?>index.php/purchase/get_all_suppliers',
            success: function(data) 
            {
               $.each(data, function (index, value) 
                {
...
$(document).on('click', '#fs-product-btn', function(){
    $.ajax({
            type: 'POST',
            dataType: 'json',
            url: '<?php echo base_url(); ?>index.php/purchase/get_all_products',
            success: function(data) 
            {
               $.each(data, function (index, value) 
                {
