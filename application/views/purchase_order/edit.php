...
<div id="page-content" class="p20 clearfix">
    ...
    ?>
      <div id="success-alert" class="alert alert-success alert-dismissible">
       ...
       <strong>Success!</strong>&nbsp;Purchase order updated successfully.
      </div>
      ...
      <?php
        ...
        <div class="">
            <div class="panel panel-default">
                ...
                <br /> 
                <?php echo form_open(get_uri("purchase_order/updatepurchase"), array("id" => "purchase-form", "class" => "general-form", "role" => "form")); ?>
                <div class="container row">
                <div class="col-md-5">
                        <div class="form-group">
                <label for="title" class=" col-md-3">Purchase Order No*</label>
                <input type="hidden" name="purchase_order_id" value="<?= $purchase_data->id; ?>"/>
                            <div class="<?php echo $field_column; ?>">
                                <div class=" col-md-8">
                                  <input type="number" name="po_no" class="form-control" value="<?= $purchase_data->po_no; ?>"  required readonly>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <label for="title" class=" col-md-3">Quotation Number</label>
                            <div class="<?php echo $field_column; ?>">
                                <div class=" col-md-8">
                                  <input type="number" name="quotation_number" class="form-control" value="<?= $purchase_data->quotation_number; ?>" >
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <label for="title" class=" col-md-3">Purchase requisition Number*</label>
                            <div class="<?php echo $field_column; ?>">
                                <div class=" col-md-8">
                                  <input type="number" name="purchase_requstion_number" class="form-control" value="<?= $purchase_data->purchase_requstion_number; ?>"  required readonly ><!--changes 22-3-->
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="table-responsive mt-3">
                    <table id="myTable" class="table table-hover table-striped order-list" style="width: 80%; font-weight: 600">
                        ...
                        <?php
                            $options = array();
                            $options['purchase_order_id'] = $purchase_data->id;//changs 22-3
                            $purchase_detail = $this->Purchaseorder_details_model->get_details($options)->result();
                            ...
                            <select name="order_tax" class="form-control">   
                                    <option value="">Select Order Tax</option> 
                                        <?php foreach($tax_all as $tx){?>
                                            <option value="<?php echo $tx->id; ?>"  <?php if($purchase_data->order_tax == $tx->id){?> selected <?php }?> ><?php echo $tx->title; ?> </option><!--changes 22-3-->
                        <?php }  ?>                                                    
                                </select>
                        </div>
...
<script type="text/javascript">
    ...
    $(document).on('click', '.selected', function(){
        ...
        else
        {
            ...
            orderlist += '<td class="unit-name">'+unit+'</td>';//changes 22-3
