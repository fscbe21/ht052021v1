...
<?php
      ...
      if($work_order_data->reuse_work_order_id){
        $options['work_order_id'] = $work_order_data->reuse_work_order_id;
      }else{
        $options['work_order_id'] = $work_order_data->id;
      }
      
      $options['end_product_id']   = $wo_product_id;
    ...
    foreach ($process_list_data as $key=>$value)
    {
      ...
      //echo form_dropdown("no_of_process", $process_array,array() ,"class='form-control select2' id='no_of_process'");
        ...
        ?>
            ...
            <select name="no_of_process" class="form-control">
                    ...
                    foreach($process_list_data as $proc){
                      ?>
                          <option 
                            <?php
                                if($noofprocess){
                                  echo ($setprocessdata[0]->no_of_process == $proc->id) ? ' selected' : ''; 
                                }
                            ?>
...
if(count($setprocessdata) > 0){
  ...
  <input name="already_ep_id" type="hidden" value="<?php echo $end_product_id; ?> ">
  <?php
}else{
  ?>
    ...
    <?php
      if(count($setprocessdata) > 0){
          foreach($setprocessdata as $spd){
            ...
            <tr>
              <td>
                  <select name="product_field[]" class="select2 form-control product-id" >
                    <?php
              ...
              <td>
                  <select name="process_field[]" class="select2 form-control">
                    <?php
              ...
              <td>
                  <select name="vendor_field[]" class="select2 form-control">
              ...
              <td class="unit-name">
                   <?php
                        $product_data = $this->Products_model->get_one($spd->end_product_id);
...
<input type="hidden" name="end_product_id" value="<?= $wo_product_id;       ?>"/>
<input type="hidden" name="wo_product_id"  value="<?= $wo_product_id;       ?>"/>
...
$( document ).ready(function() {
  ...
  $("#no_of_process").select2().on("change", function () {
            ...
            for (let i = 1; i <= $(this).val(); i++) { 
              ...
              <td><?php echo form_dropdown("product_field[]", $product_array,array() ,"class='select2 product-id form-control'");?></td>
