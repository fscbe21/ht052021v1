                ...
                ?>
                    ...
                    <?php
                        foreach($set_process_data as $spd){
                            $endproductdata = $this->Products_model->get_one($spd->wo_product_id);
                            ...
                            <input type="hidden" name="wo_product_id[]" value="<?= $spd->end_product_id; ?>">
                            ...
                            <?php
                                ...
                                $options                   = array();
                                $options['work_order_id']  = $work_order_data->id;
                                $options['wo_product_id']  = $spd->wo_product_id;
                                ...
                                if(count($stage_data) > 0){
                                    ...
                                    $wodata = array();
                                    $wodata = $this->Work_order_model->get_one($work_order_data->id);
                                    $option = array();
                                    $option['work_order_id'] = $wodata->reuse_work_order_id;
                                    $option['product_id']    = $spd->end_product_id;
                                    $reuse_details           = array();
                                    $reuse_details           = $this->Reuse_details_model->get_details($option)->result();

                                    $reusestage = $reuse_details->stage;
                                    $options                   = array();
                                    $options['work_order_id']  = $wodata->reuse_work_order_id;
                                    $options['end_product_id'] = $spd->end_product_id;
                                    $options['process_id']     = $spd->process_id;
                                    $options['stage']          = $reusestage;
