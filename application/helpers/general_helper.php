...
if (!function_exists('add_auto_reply_to_ticket')) {
    ...
    if(! function_exists('get_random_number')){
        ...
        function get_random_number($digit=8, $unique='products'){
            $random_number = 0;
            $repeat = 1;
            $ci = get_instance();
            $checkCode = 1;
            ...
            do{
              $random_number =  mt_rand($start_number, $end_number);

              if($unique == 'products'){
                 $checkCode     = $ci->Products_model->check_product_code($random_number);
              }   
    ...
    if(! function_exists('code_exists')){
        ...
        function code_exists($code){
            $ci = get_instance();
            return $ci->Products_model->check_product_code($code);
        }
    }
    ...
    if(! function_exists('payment_status')){
        ...
        function payment_status($id){
            $ci = get_instance();
            return $ci->Payment_status_model->get_name($id);
        }
    }
...
if (!function_exists('get_product_name')) {
    ...
    function get_product_name($product_id = 0) {
        $ci = get_instance();
        $product_data = array();
        $product_data = $ci->Products_model->get_one($product_id);

        return $product_data->name;
    }
}

if (!function_exists('get_bom_name')) {

    function get_bom_name($bom_id = 0) {
        $ci = get_instance();
        $bom_data = array();
        $bom_data = $ci->Bom_model->get_one($bom_id);

        return $bom_data->name;
    }
}
