...
//darini 19-2
if (!function_exists('count_unread_message1')) {

    function count_unread_message1() {
        $ci = get_instance();
        return $ci->Mom_model->count_unread_message($ci->login_user->id);
    }
...
if (!function_exists('total_leads_widget')) {
    ...
    function total_leads_widget($return_as_data = false, $show_own_clients_only_user_id = "") {
        $ci = get_instance();
        $view_data["total_leads"] = $ci->Clients_model->count_total_leads($show_own_clients_only_user_id);
        return $ci->load->view("clients/total_leads_widget", $view_data, $return_as_data);
    }

}


if (!function_exists('count_open_lead')) {

    function count_open_lead($show_own_clients_only_user_id = "", $type = "") {
        $ci = get_instance();
        return $ci->Clients_model->count_open_lead($show_own_clients_only_user_id, $type);
    }

}


if (!function_exists('sales_target')) {

    function sales_target($return_as_data = false) {
        $ci = get_instance();
        ...
        return $ci->load->view("events/sales_target", $view_data, $return_as_data);
    }

}

if (!function_exists('target_achieved')) {

    function target_achieved($return_as_data = false) {
        $ci = get_instance();
        ...
        return $ci->load->view("events/target_achieved", $view_data, $return_as_data);
    }

}

if (!function_exists('performance_chart')) {

    function performance_chart($return_as_data = false) {
        $ci = get_instance();
        ...
        return $ci->load->view("events/performance_chart", $view_data, $return_as_data);
    }

}

if (!function_exists('type_of_materials')) {  // AG12022021

    function type_of_materials() {
       // $ci = get_instance();
      //  $owner_id = $ci->login_user->id;
      //  $view_data["events"] = $ci->Clients_model->materials_info($owner_id);
         $view_data  = array();
        return $ci->load->view("events/material_info", $view_data);
    }
