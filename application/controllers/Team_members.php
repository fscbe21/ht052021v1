...
//R.V01_03Ey
    ...
    function approve($id) {
        $this->Users_model->approve_user($id);
        redirect("team_members");
 
     }

     function decline($id) {
       $this->Users_model->decline_user($id);
       redirect("team_members");
...
//R.V26_03_S
   ...
   /* function job_info($user_id) {
       ...
       $this->update_only_allowed_members($user_id);
        ...
        $view_data['job_info']->job_title = $user_info->last_job_title;
        $view_data['user_info'] = $this->Users_model->get_one($user_id);//R.v25_03
    ...
    function job_info($user_id) {
        ...
        $options = array("id" => $user_id);
        $user_info = $this->Users_model->get_details($options)->row();
        $view_data['salary_info'] = $this->Users_model->get_job_info($user_id);;
...
//R.V01_03E
    ...
    function save_account_settings($user_id) {
        ...
        if ($this->login_user->is_admin && $this->login_user->id != $user_id) {
            ...
            $account_data['bank_acc_no'] = $this->input->post('bank_acc_no');
            $account_data['account_type'] = $this->input->post('account_type');
            $account_data['acc_holder_name'] = $this->input->post('acc_holder_name');
            $account_data['ifsc_code'] = $this->input->post('ifsc_code');
            $account_data['bank_name'] = $this->input->post('bank_name');

            $account_data['branch_name'] = $this->input->post('branch_name');
    ...
    function get_user_details($user_id){
        ...
        $user_info =  $this->Users_model->get_details(array("id" => $user_id))->row();
      ...
      function proof_download($user_id=0){

        $view_data["user_id"] = $user_id;
        $options = array();
        $options['user_id'] = $user_id;
        $user_data = $this->Users_model->get_one($user_id);

        $proof = $user_data->id_files;
        $view_data['files'] = unserialize($proof);

        $this->load->view("team_members/proof_download", $view_data);
      }

      function download_files($id) {
        $files = $this->Users_model->get_one($id)->id_files;
