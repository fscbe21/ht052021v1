...
class Clients extends MY_Controller {
    ...
    function modal_form() {
        ...
        $view_data['districts'] = $this->Other_district_model->get_details()->result();
  ...
  /*AG040321*/"payment_due_date" => $this->input->post('payment_due_date'),
            "comments" => $this->input->post('comments'),//end
        );
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $total_work_orders = 0;
        $option = array();
        $option['customer_id'] = $data->id;
        $total_work_data = $this->Work_order_model->get_details($option)->result();
        $total_work_orders = count($total_work_data);


        $invoice_data   = array();
        $invoice_amount = 0;
        $opt            = array();
        $opt['client_id'] = $data->id;
        $invoice_data = $this->Invoices_model->get_details($opt)->result();
        $payments = 0;
        foreach($invoice_data as $ind){
            $optt = array();
            $optt['invoice_id'] = $ind->id;
            $invoice_item_detail = array();
            $invoice_item_detail  = $this->Invoice_items_model->get_details($optt)->result();
            $invoice_amount = 0;
            foreach($invoice_item_detail as $inid){
                $invoice_amount += $inid->total;
            }

            $payments_data = array();
            $opp           = array();
            $opp['invoice_id'] = $ind->id;
            $payments_data = $this->Invoice_payments_model->get_details($opp)->result();
            $payments = 0;
            foreach($payments_data as $pdata){
                $payments += $pdata->amount;
            }
        }
        ...
        $userdata    = $this->Users_model->get_one($data->owner_id);
        ...
        $row_data = array();
        $row_data[] = $data->id;
        $row_data[] = anchor(get_uri("clients/view/" . $data->id), $data->company_name);
        if($this->login_user->role_id != 3){
            $row_data[] = $fullname;
        }
        $row_data[] = $data->primary_contact ? $primary_contact : "";
        $row_data[] = $group_list;
        $row_data[] = $total_work_orders;
        $row_data[] = to_currency($invoice_amount, $data->currency_symbol);
        $row_data[] = to_currency($payments, $data->currency_symbol);
        $row_data[] = to_currency($invoice_amount - $payments, $data->currency_symbol);
        if($due > 0) 
        { 
            $date = $data->payment_due_date ? $data->payment_due_date : '-';
            if($date != '-')
            {
    ...
    /*AG040321*/
    function payment_due(){
        $this->template->rander("clients/payments_due_list");
    }
    ...
    function company_info_tab($client_id = 0) {
        ...
        if ($client_id) {
            ...
            $view_data['statuses'] = $this->Lead_status_model->get_details()->result();//darini 20-2
    ...
    function payment_info_form() {
        ...
        $this->load->view('clients/payment_info_form_view', $view_data);
    }

    /* load new work  tab  */

    function new_work_order($client_id) {
        $this->access_only_allowed_members();
        $this->can_access_this_client($client_id);

        
        $view_data["custom_field_headers"] = $this->Custom_fields_model->get_custom_field_headers_for_table("new work order", $this->login_user->is_admin, $this->login_user->user_type);

        $view_data['client_id'] = $client_id;
        $this->load->view("clients/new_work_order/index",$view_data );
    }

    /* load  new work tab  */

    function production_status($client_id) {
        $this->access_only_allowed_members();
        $this->can_access_this_client($client_id);
        ...
        $view_data['client_id'] = $client_id;
        $this->load->view("clients/production_status/index",$view_data );
    }

    //darini 10-4
    function advance($client_id) {
        $this->access_only_allowed_members();
        $this->can_access_this_client($client_id);
        ...
        $view_data['client_id'] = $client_id;
        $this->load->view("clients/advance/index",$view_data );
    }
    //end
    function advance_list($id){
        $list_data=$this->Advance_model->getadvancelist($id);
        $result = array();
       foreach ($list_data as $data) {
            $result[] = $this->_make_row_advance($data);
        }
        echo json_encode(array("data" => $result));
    }
    function _make_row_advance($data){
        $client_info = $this->Clients_model->get_one($data->client_id);
        $row_data = array(
            " ",
            ...
            $data->advance_date,           
            $data->advance_notes,
            modal_anchor(get_uri("clients/edit_advance"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => 'Edit Advance', "data-post-id" => $data->id,"data-post-client_id"=>$data->client_id))
     ...
     function add_advance(){
        ...
        $view_data['label_column'] = "col-md-3";
        $view_data['field_column'] = "col-md-9";
        $this->load->view('clients/advance/add_advance', $view_data);
     }

     function edit_advance(){
        $id=$this->input->post('id');
        $view_data['model_info'] =  $this->Advance_model->get_one( $id);
        $view_data['client_id'] = $this->input->post('client_id');
        $view_data['label_column'] = "col-md-3";
        $view_data['field_column'] = "col-md-9";
        $this->load->view('clients/advance/add_advance', $view_data);
     ...
     function save_advance(){
        ...
        $data['advance_date']  = $this->input->post('advance_date');
        $data['advance_notes']  = $this->input->post('advance_notes');
       
        $save_id = $this->Advance_model->save($data,$id);
        ...
        if($save_id){
            ...
            if($id){
                ...
                $pre_adamt=$this->input->post('pre_adamt');
                $totalamtwhole=$client_info->total_advance_amount-$pre_adamt;
                $totalamt=floatval($totalamtwhole)+floatval($data['advance_amount'] );
                $update=array();
                $update['total_advance_amount']=$totalamt;
                $save_id1 = $this->Clients_model->save($update, $client_id);

            }else{
            $totalamt=floatval($client_info->total_advance_amount)+floatval($data['advance_amount'] );
     ...
     function get_all_clients(){
        $data = array();
        $data = $this->Clients_model->get_al()->result();
        $newdata = array();
        foreach($data as $d){
            $newdata['id'] = $d->id;
        }
