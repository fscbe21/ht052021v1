...
class Invoice_payments extends MY_Controller {
    ...
    function save_otp_payment(){
        $invoice_payment_id = $this->input->post('id');
        ...
        if($otp == 123456)
        {
            $data = array();
            $data['otpstatus'] = 1;
            $save_data = $this->Invoice_payments_model->save($data, $invoice_payment_id);
            log_notification("invoice_payment_confirmation", array("invoice_payment_id" => $invoice_payment_id, "invoice_id" => $invoice_id), "0");
            echo json_encode(array("success" => true, 'message' => 'Payment verified'));
        }
        else
        {
            echo json_encode(array("success" => false, 'message' => "Invalid OTP, Please try again later!"));
        }
    }

    function payment_otp_form() {
        $this->access_only_allowed_members();
        ...
        $this->load->view('invoices/payment_otp_form', $view_data);
    }
    ...
    function save_payment() {
        ...
        $invoice_payment_data = array(
            ...
            "post_date"=>$this->input->post('post_date')//darini 12-2
        );
        ...
        if(count($filenames) > 0){
            ...
            $files_data = move_files_from_temp_dir_to_permanent_dir($target_path, "invoicepayment");
            $image_data = unserialize($files_data);
            $invoice_payment_data['image'] = $image_data[0]['file_name'];
        }
        ...
        if ($invoice_payment_id) {
            ...
            if($this->input->post('invoice_payment_method_id')==6){
                $invoice=$this->Invoices_model->get_one($invoice_id);
                $client_info=$this->Clients_model->get_one($invoice->client_id);
                $totalamt=floatval($client_info->total_advance_amount)-(floatval($this->input->post('invoice_payment_amount'))-floatval($this->input->post('previous_amount')));
                $update=array();
                $update['total_advance_amount']=$totalamt;
                $save_id1 = $this->Clients_model->save($update, $invoice->client_id);
            }//end
    ...
    private function _make_payment_row($data) {
        ...
        return array(
            ...
            ($otp == 0)
             ?
              "<span class='text-danger'".modal_anchor(get_uri("invoice_payments/payment_otp_form"), "Unverified", array("class" => "edit", "title" => "Verify Invoice Payment", "data-post-id" => $data->id, "data-post-invoice_id" => $data->invoice_id,))."</span>"
              :
