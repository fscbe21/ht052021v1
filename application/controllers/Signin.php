...
class Signin extends CI_Controller {
    ...
    function index() {
        if ($this->Users_model->login_user_id()) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data['login_time'] = get_my_local_time();
                $data['ip_address'] = get_real_ip();
                $this->Logs_model->save($data);
    ...
    function sign_out() {
        $logout_time = get_my_local_time();
        $user_id = $this->session->user_id;
        $ip = get_real_ip();
        $this->Logs_model->set_logout_time($user_id, $ip, $logout_time);
