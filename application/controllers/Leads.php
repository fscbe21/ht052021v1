...
class Leads extends MY_Controller {
    ...
    function visit_info_tab($client_id){
        ...
        if ($client_id) {
            ...
            $this->load->view('leads/contacts/visit_info_tab', $view_data);
        }

    }


    function save_lead_visit_info(){
        //darini 10-2
      $data=array(
            ...
            'owner_id'          => $this->input->post('owner_id'), //extra line darini 10-2
            'followup_through'  =>$this->input->post('followup_through')//extra line darini 11-2
        ...
        $update_lead = array(
            ...
            'time'              => convert_time_to_24hours_format($this->input->post('time')),
            'total_value'       => $this->input->post('total_value'),
            'nextmeet'          => $this->input->post('followup_date')." ".convert_time_to_24hours_format($this->input->post('time'))               
        );

        $leadId = $this->input->post('lead_id');

        $savedata = $this->Clients_model->save($update_lead, $leadId);

        $lead_visit_info_id =  $this->Lead_visit_model->save($data);

        if($lead_visit_info_id){
            echo json_encode(array("success" => true, 'message' => lang('record_saved'), 'lead_status_id'=>$this->input->post('lead_status_id') ));  
        }else{
    ...
    function modal_form_info($lead_id) {
        ...
        $data['info'] = $this->Clients_model->get_one($lead_id);
        $data['visit_info'] = $this->Lead_visit_model->get_details(array("lead_id"=>$lead_id))->result();
        //darini 9-2-21
       ...
       // $data['visit_info'] = $lead_source->title;
        ...
        $this->load->view('leads/modal_form_info', $data);
    }
    ...
    function save() {
       ...
       if($this->login_user->role_id==3){
        if(!$client_id){
             ...
             $save_id = $this->Lead_approval_model->save($data);
        }
    ...
    //end
        if ($save_id) {
            ...
            echo json_encode(array("success" => true, "data" => $this->_row_data($save_id), 'id' => $save_id, 'view' => $this->input->post('view'), 'message' => lang('record_saved'),'lead_status_id'=>$this->input->post('lead_status_id')));
        } else {
            echo json_encode(array("success" => false, 'message' => "Some Error occured !"));
        }
    }
    ...
    */
        ...
        $row_data[] = modal_anchor(get_uri("leads/modal_form_info/".$data->id), "<span class='companyname'>".$data->company_name."</span>", array("class" => "leadTimeline", "title" => lang('lead_timeline'), "data-id" => $data->id, "data-companyname" => $data->company_name));
        ...
        $row_data[] = "<small style='padding: 4px;background-color: ".$lead_status->color."'>".$lead_status->title."</small>";
        //$row_data[] = $data->owner_id;//ANAND11022021
        $owner_data = $this->Users_model->get_one($data->owner_id);//ANAND11022021

        if($this->login_user->role_id != 3){
            $row_data[] = $owner_data->first_name." ".$owner_data->last_name; //ANAND11022021
        }

        $row_data[] = $lead_source->title;
        $row_data[] = $data->branch;
        $row_data[] = dateFormatChange($data->created_date);
        $row_data[] = dateFormatChange($data->followup_date).'<br />'.$data->time;
        $row_data[] = dateFormatChange($data->created_date);
        $row_data[] = $data->company_unit;        
    ...
    function all_leads_kanban_data() {
        ...
        $view_data['upcoming'] = $this->Lead_visit_model->get_upcoming_visit_detail();
        $view_data['pending'] = $this->Lead_visit_model->get_pending_visit_detail();
        $view_data['discussion'] = $this->Lead_visit_model->get_discussion_visit_detail();
        $view_data['negotiation'] = $this->Lead_visit_model->get_negotiation_visit_detail();
        $view_data['wonlist'] = $this->Lead_visit_model->get_won_visit_detail();
    ...
    function make_client_modal_form($lead_id = 0) {
        ...
        $view_data['team_members'] = $this->Users_model->team_members();//darini 19-2
  //R.V15_04_21S
        $view_data['roads'] = $this->Other_road_model->get_details()->result();
        $view_data['areas'] = $this->Other_area_model->get_details()->result();
        $view_data['districts'] = $this->Other_district_model->get_details()->result();//R.V15_04_21E
    ...
    function save_as_client() {
        ...
        if ($client_id) {
            ...
            $data['last_approved'] = $this->login_user->role_id;
            date_default_timezone_set('Asia/Kolkata');
            $now = date('Y-m-d H:i:s');
            $isLead = 1;
            if($this->login_user->role_id == 1){
                $data['is_gm_approved'] = 1;
                $data['gm_approved_time'] = $now;
                $data['is_lead'] = 1;
            }else if($this->login_user->role_id == 2){
                $data['is_sm_approved'] = 1;
                $data['sm_approved_time'] = $now;
                $data['is_lead'] = 1;
            }else if($this->login_user->role_id == 3){
                $data['is_se_approved'] = 1;
                $data['se_approved_time'] = $now;
                $data['is_lead'] = 1;
            }else{
                $data['is_md_approved'] = 1;
                $data['md_approved_time'] = $now;
                $data['is_lead'] = 0;
                $isLead = 0;
            }
                ...
                foreach ($contacts as $contact) {
                    ...
                    if ($save_contact_id) {
                        ...
                        if ($this->input->post('email_login_details-' . $contact->id)) {
                            ...
                            $parser_data["CONTACT_FIRST_NAME"] = $user_data["first_name"];
                            //$parser_data["USER_LAST_NAME"] = $user_data["last_name"];
                            $parser_data["CONTACT_LOGIN_EMAIL"] = $user_data["email"];
                            $parser_data["CONTACT_LOGIN_PASSWORD"] = $this->input->post('login_password-' . $contact->id);
                            $parser_data["COMPANY_NAME"] = "Danam Processing";
            ...
            if($save_client_id && ($this->login_user->role_id != 3)){
                if($isLead == 0){
                    echo json_encode(array("success" => true, 'redirect_to' => get_uri("client_approval"), "message" => lang('record_saved')));
                }else{
                    echo json_encode(array("success" => true, 'redirect_to' => get_uri("leads/view/").$client_id, "message" => lang('record_saved')));
                }
                
                
            } 
            else if($save_client_id && ($this->login_user->role_id == 3)){
                ...
                if((! $save_client_id) && ($this->login_user->role_id != 3)){
                    ...
                    echo json_encode(array("success" => false, 'redirect_to' => get_uri("leads/view/").$client_id, "message" => lang('error_occurred')));
                }
            }
        }
    }
    ...
    function check_company_name($company_name){
        ...
        $result = $this->Clients_model->company_name_details($company_name)->result();
    ...
    function get_branch_details($branch_details,$company_name){

        $result = $this->Clients_model->company_branch_details($branch_details,$company_name)->result();
     ...
     function get_unit_details($unit,$branch,$company_name){

        $result = $this->Clients_model->company_unit_details($unit,$branch,$company_name)->result();
     ...
     function  updatetransfer($id){
        $owner_id=$this->input->post('transfer_to');
        $savedata = $this->Clients_model->updateclientowner($owner_id,$id);
        echo json_encode(array("success" => true, 'message' => lang('record_updated')));
        //echo  $owner_id;
     }
     //end
 //darini12-2
 function modal_form1($lead_id1) {
    $lead_id = $this->input->post('id');
    
    $this->can_access_this_lead($lead_id);
    $view_data = $this->make_lead_modal_form_data1($lead_id,$lead_id1 );
    $view_data['team_members'] = $this->Users_model->team_members();
    $view_data['leadapp']="yes";
    $this->load->view('leads/modal_form', $view_data);
}


private function make_lead_modal_form_data1($lead_id=0,$lead_id1) {
    $this->access_only_allowed_members();
    ...
    $view_data["view"] = $this->input->post('view'); //view='details' needed only when loding from the lead's details view
    $view_data['model_info'] = $this->Lead_approval_model->get_one($lead_id1);
    $view_data["currency_dropdown"] = $this->_get_currency_dropdown_select2_data();
    $view_data["owners_dropdown"] = $this->_get_owners_dropdown();
