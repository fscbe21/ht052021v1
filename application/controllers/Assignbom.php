...
class Assignbom extends MY_Controller {
    ...
    private function _make_row($data) {
        ...
        /* $stages_data  = $this->Other_stage_model->get_one($data->stages);
        $stages_name  = $stages_data->id; */

        $unit_data    = $this->Unit_model->get_one($unit_id);
        $unit_name    = $unit_data->name;

        $bom_list     = "<select name='bom_name[]' class='select2 form-control'>";
        $bom_found    = 0;
        $bomid        = 0;
        foreach($bom_array as $bom){

            if(strtolower(trim($bom->name)) == strtolower(trim($product_name))){
                $bom_list .= "<option value='".$bom->id."'>".$bom->name."</option>";
                $bom_found = 1;
                $bomid = $bom->id;
            }
        }
        ...
        return array(
            ...
            modal_anchor(get_uri("assignbom/modal_form"), "<i class='fa fa-eye'></i>", array("class" => "edit", "title" => "View & Assign BOM", "data-post-endproductid" => $data->end_product_id, "data-post-workorderid" => $data->work_order_id,"data-post-woproductid" => $data->wo_product_id, "data-post-bomid" => $bomid, "data-post-qty" => $wodetails[0]->qty)).$bomassigned
