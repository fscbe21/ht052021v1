...
class Logistics extends MY_Controller {
    ...
    function list_data() {
        ...
        $list_data = $this->Vechicle_model->get_details($options)->result();
        $result = array();
        $sl = 1;
    ...
    private function _make_row($data, $sl) {
        ...
        if($data->deleted == 1){
            $delete = "<a href='".base_url()."index.php/logistics/approve/".$data->id."'><i class='fa fa-check'></i></a>";
        }else{
    ...
    function approve($id){
        if($id){
            $this->Vechicle_model->undo_delete($id);
        }
