...
class Invoices extends MY_Controller {
    ...
    function modal_form() {
        ...
        if ($project_id) {
            $client_id = $this->Work_order_model->get_one($project_id)->customer_id;
        ...
        $projects = $this->Work_order_model->get_dropdown_list(array("id"), "id", array("customer_id" => $project_client_id));
    ...
    function get_project_suggestion($client_id = 0) {
        ...
        $projects = $this->Work_order_model->get_dropdown_list(array("id"), "id", array("customer_id" => $client_id));

        $suggestion = array(array("id" => "", "text" => "-select work order -"));
    ...
    function get_project_suggestion1($client_id, $invoice_type) {
        ...
        foreach ($projects as $key => $value) {
            ...
            $invCount = $this->Invoices_model->get_details($options)->result();
            $proceed = 0;
            $proceed = $this->get_work_order_details($value);
            if($invoice_type == 1){
                if(count($invCount) == 0){
                    if($proceed == 1){
                        $suggestion[] = array("id" => $key, "text" => $value);
    ...
    function get_work_order_details($work_order_number){
        $options = array();
        $options['work_order_id'] = $work_order_number;
        $work_order_details = $this->Work_order_details_model->get_details($options)->result();

        $return = 0;

        if(count($work_order_details) > 0){

            $total = count($work_order_details);
            $rowCount = 0;
            
            foreach($work_order_details as $wod){
                $option = array();
                $option['work_order_id'] = $work_order_number;
                $option['end_product_id']    = $wod->product_id;
                $option['status']            = 1;
                $process_data = $this->Set_process_model->get_details($option)->result();
    ...
    function save() {
        ...
        $invoice_ttype = $this->input->post('invoice_ttype');


        $invoice_data = array(
            "client_id" => $client_id,
            "project_id" => $this->input->post('invoice_project_id') ? $this->input->post('invoice_project_id') : 0,
            "bill_no" => $this->input->post('bill_no') ? $this->input->post('bill_no') : 0,
            "bill_date" => $bill_date,
            "due_date" => $this->input->post('invoice_due_date'),
            "dc_no" => $this->input->post('dc_no') ? $this->input->post('dc_no') : 0,
            "po_name" => $this->input->post('po_name'),
            "po_date" => $this->input->post('po_date'),
            "add_labour_cost" => $this->input->post('add_labour_cost'),
            ...
            "weight_slip_no" => $this->input->post('weight_slip_no'),//changes 29-3
            "date" => $this->input->post('date'),
            "empty_weight" => $this->input->post('empty_weight'),
            "full_weight" => $this->input->post('full_weight'),
            "job_weight" => $this->input->post('job_weight'),
            "driver_name" => $this->input->post('driver_name'),
            "vechicle_no" => $this->input->post('vechicle_no'), 
            "contact_no" => $this->input->post('contact_no'), //changes 29-3 end            
            "consigne_name" => $this->input->post('consigne_name'), //changes 1-4
            "consigne_add_line1" => $this->input->post('consigne_add_line1'),
            "consigne_add_line2" => $this->input->post('consigne_add_line2'),
            "phone_no" => $this->input->post('phone_no'),
            "gstin" => $this->input->post('gstin'),//end
        );
        ...
        if ($invoice_id) {
            ...
            $item_data = array();
            $opt = array();
            $opt['work_order_id'] = $this->input->post('invoice_project_id'); 
            $item_data = $this->Work_order_details_model->get_details($opt)->result();

            if($invoice_ttype == 2){
                $opp                = array();
                $opp['notinclude']  = $invoice_id;
                $opp['project_id']  = $this->input->post('invoice_project_id');

                $already_found      = array();
                $already_found      = $this->Invoices_model->get_details($opp)->result();
                
                if(count($already_found) > 0){
                    foreach($item_data as $item){
                        $product_data  = array();
                        $product_data  = $this->Products_model->get_one($item->product_id);
                        ...
                        foreach($already_found as $af){
                            $opppp     = array();
                            $opppp['invoice_id'] = $af->id;
                            $opppp['product_id'] = $item->product_id;
                            $invoice_itemsdata   = array();
                            $invoice_itemsdata   = $this->Invoice_items_model->get_details($opppp)->result();

                            foreach($invoice_itemsdata as $iid){
                                $product_qty   += $iid->quantity;
                            }

                            if($product_qty < $item->qty){
                                $inv_qty       = 0;
                                $inv_qty       =  $item->qty - $product_qty;
                                ...
                                $invoice_item_data['product_id'] = $product_data->id;
                                $invoice_item_data['title']      = $product_data->name;
                                $invoice_item_data['unit_type']  = $unit_data->name;
                                $invoice_item_data['rate']       = $item->cost;
                                $invoice_item_data['total']      = $item->cost * $inv_qty;
                                $invoice_item_data['quantity']   = $inv_qty;
                                $invoice_item_data['invoice_id'] = $invoice_id;
                    ...
                    foreach($item_data as $item){
                       $wod = array();
                       $wod['work_order_id'] = $item->work_order_id;
                       $wod['wo_product_id'] = $item->product_id;
                       $wod['status']        = 1;
                       $setprocessdata       = array();
                       $setprocessdata       = $this->Set_process_model->get_details($wod)->result();
                       if(count($setprocessdata) == 1){
                            $product_data    = array();
                            $product_data    = $this->Products_model->get_one($item->product_id);
                            ...
                            $invoice_item_data['unit_type']  = $unit_data->name;
                            $invoice_item_data['rate']       = $item->cost;
                            $invoice_item_data['total']      = $item->total_cost;
                            $invoice_item_data['quantity']   = $item->qty;
                            $invoice_item_data['invoice_id'] = $invoice_id;
                            $this->Invoice_items_model->save($invoice_item_data);
                        }
                    }
                ...
                $this->Invoice_items_model->delete_old_entries($invoice_id);
                foreach($item_data as $item){
                    $product_data  = array();
                    $product_data  = $this->Products_model->get_one($item->product_id);
    ...
    private function _make_item_row($data) {
        ...
        if ($this->can_edit_invoices()) {
            ...
            $editdis= "<span >".modal_anchor(get_uri("invoices/item_discount_modal"), "<i class='fa fa-pencil'></i>", array("class" => "edit ", "title" => "Edit Discount", "data-post-id" => $data->id))."<span>";//2-4
        }
        $item = "<div class='item-row strong mb5' data-id='$data->id'>$move_icon $data->title$editdis</div>";//2-4
        if ($data->description) {
        ...
        return array(
            ...
            to_currency($data->discount, $data->currency_symbol), //2-4
    ...
    function file_preview($id = "", $key = "") {
     ...
     //darini 28-3
     function geninvoice($invoice_id = 0) {
        if ($invoice_id) {
            $view_data = get_invoice_making_data($invoice_id);
            $view_data['id']=$invoice_id;
            $view_data['biller_data']=$this->Biller_model->get_all()->result();           
            $view_data["invoice_total_summary"] = $this->Invoices_model->get_invoice_total_summary($invoice_id);
           
            //echo json_encode(array("success" => true, "view" => $view_data));
           $this->template->rander("invoices/geninvoice", $view_data);
        } else {
            show_404();
        }
    }
    //2-4
    function item_discount_modal(){
        if (!$this->can_edit_invoices()) {
            redirect("forbidden");
        }
        ...
        $view_data['model_info'] = $this->Invoice_items_model->get_one($id);

        $this->load->view('invoices/item_discount_modal_form', $view_data);
    }

    function save_discount_item(){
        $id = $this->input->post('id');
        $invoice_id = $this->input->post('invoice_id');
        $discount_amount = $this->input->post('discount_amount');        
        $discount_amount_type = $this->input->post('discount_amount_type');
        ...
        if($discount_amount_type=="percentage"){
            $amount=($discount_amount * $total_amount )/100;
        ...
        $save_id= $this->Invoice_items_model->save($update, $id);
