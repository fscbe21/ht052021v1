...
class Sales_qutation extends MY_Controller {
    ...
    function index(){
        ...
        $this->template->rander("sales_qutation/index",$view_data);
    }
    ...
    function add_sales(){
        ...
        $this->template->rander("sales_qutation/add_sales_qutation",$view_data);  
    }

    function edit($sales_id){

        $view_data['info']=$this->SalesQuotationModel->get_one($sales_id);
        $view_data['warehouse_all'] = $this->Warehouse_model->get_all()->result();
        $view_data['supplier_all'] = $this->Supplier_model->get_all()->result();
        $view_data['tax_all'] = $this->Taxes_model->get_all()->result();
        $view_data['cust_all'] = $this->Clients_model->client_list();
        $view_data['bill'] = $this->Biller_model->get_all()->result();
        $view_data['payment_method']=$this->Payment_methods_model->get_all()->result();
        $this->template->rander("sales_qutation/edit",$view_data);  
    }

    function list_data(){
        $list_data = $this->SalesQuotationModel->sales_quotation_list();
        $result = array();
  ...
  function _make_row($data){
      ...
      $edit='<li role="presentation">' .'<a class="edit" href="sales_qutation/edit/'.$data->id.'"><i class="fa fa-pencil"></i>Edit</a></li>';
      $view='<li role="presentation">' .modal_anchor(get_uri("sales_qutation/modal_form/".$data->id), "<i class='fa fa-eye'></i>View Sales Order", array("class" => "edit", "title" => "View Sales detail", "data-post-id" => $data->id)).'</li>';
      $delete='<li role="presentation">' .js_anchor("<i class='fa fa-times fa-fw'></i> ". lang('delete'), array('title' => "Delete Sales Order", "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("sales_qutation/delete"), "data-action" => "delete-confirmation")).'</li>';
  ...
  function save(){
   ...
   /*  echo "<pre>";
    ...
    $data = array();
    $data['reference_no'] = 'sqr-' . date("Ymd") . '-'. date("his");
    ...
    if(count($filenames) > 0){            
        $target_path = get_setting("timeline_file_path_sales_quotation");
        $files_data = move_files_from_temp_dir_to_permanent_dir($target_path, "sales_quotation");
        $image_data = unserialize($files_data);
        $data['document'] = $image_data[0]['file_name'];
    }//end

    $save_id = $this->SalesQuotationModel->save($data);
    ...
    for($j = 0; $j < count($item_qty); $j++){
        ...
        $id=$this->ProductSalesQuotation->save( $sales_data);
...
function update_sales(){
   ...
   // echo $salesid.$old_wid;
    ...
    if(count($filenames) > 0){    
        if($oldfilename != NULL)
        {
            unlink('./files/timeline_files/sales_quotation/'.$oldfilename);
        }
   ...
   $save_id = $this->SalesQuotationModel->save($data,$salesid);
    ...
    $product_sales_data = $this->ProductSalesQuotation->salesquotation_product_list( $salesid); 
    ...
    foreach($product_sales_data as $bdata){
        ...
        $this->ProductSalesQuotation->deleteProductSalesQuotation($bdata->id);
    }
...
{
    $view_data = array();        
    $view_data['model_info'] = $this->SalesQuotationModel->get_one($save_id);
    $this->load->view('sales_qutation/modal_form', $view_data);
}
...
function delete() {
    ...
    $wid = $this->SalesQuotationModel->get_one($id);
    $product_sales_data =$this->ProductSalesQuotation->salesquotation_product_list( $id); 
    ...
    if ($this->SalesQuotationModel->deleteSalesQuotation($id)) {
