...
class Projects extends MY_Controller {
    ...
    //AG2703QQ
       ...
       $list_data = $this->Work_order_model->get_details($options)->result();
        $result = array();
        foreach ($list_data as $data) {
            $info      = $this->_make_row($data, $custom_fields, $production_status);
            if($info){
                $result[] = $this->_make_row($data, $custom_fields, $production_status);
            }
        }
        echo json_encode(array("data" => $result));
    }
    ...
    private function _make_row($data, $custom_fields, $production_status) {
        ...
        $dateline = is_date_exists($data->end_date) ? format_to_date($data->end_date, false) : "-";

        $price = $data->total_amount ? to_currency($data->total_amount, $data->currency_symbol) : "-";

        //has deadline? change the color of date based on status
        if (is_date_exists($data->end_date)) { //&& $data->status === "open"
            if ($progress !== 100  && get_my_local_time("Y-m-d") > $data->end_date) {
                $dateline = "<span class='text-danger mr5'>" . $dateline . "</span> ";
            } else if ($progress !== 100 && get_my_local_time("Y-m-d") == $data->end_date) {
                $dateline = "<span class='text-warning mr5'>" . $dateline . "</span> ";
            }
        }

        $title = anchor(get_uri("work_order/edit/" . $data->id), $data->title);
        ...
        $view_status_detail = "";
        if($this->login_user->is_admin){
            $view_status_detail = modal_anchor(get_uri("viewproduction/modal_form"), "<i class='fa fa-eye'></i>", array("class" => "edit", "title" => "View", "data-post-workorderid" => $data->id));
        }

        $row_data = array(
            anchor(get_uri("work_order/edit/" . $data->id), $data->id),
            $title,
