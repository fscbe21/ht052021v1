...
class Payment_status extends MY_Controller {
    ...
    function index() {
        $this->load->view("payment_status/index");
    }
    ...
    function save() {
        ...
        if (!$id) {
            //get sort value
            $max_sort_value = $this->Payment_status_model->get_max_sort_value();
    ...
    function update_field_sort_values($id = 0) {
        ...
        if ($sort_values) {
            ...
            foreach ($sort_array as $value) {
                ...
                $this->Payment_status_model->save($data, $id);
            }
        }
    }
    ...
    private function _make_row($data) {
        $edit = modal_anchor(get_uri("payment_status/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('payment_status'), "data-post-id" => $data->id));

        $delete = js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('payment_status'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("payment_status/delete"), "data-action" => "delete-confirmation"));
