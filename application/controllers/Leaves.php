...
/*AG2302*/  "last_return_date" => $this->input->post('last_return_date'),
/*AG2302*/  "period_of_last_leave" => $this->input->post('period_of_last_leave'),
/*AG2302*/  "leave_address" => $this->input->post('leave_address'),
/*AG2302*/  "performing_person_name" => $this->input->post('performing_person_name'),
/*AG2402*/  "last_leave_type_id" => $this->input->post('last_leave_type_id'),
            "created_at" => $now,
            "total_hours" => $hours,
            "total_days" => $days
        );
    ...
    private function _make_row($data) {
        $applicatoin_info = $this->Leave_applications_model->get_one($data->id);
        //darini
        $approval=array();
       ...
       //darini 22-2
        ...
        return array(
            ...
            $applicatoin_info->created_at, /*AG2302*/
    ...
    function update_status() {
        ...
        $applicatoin_info = $this->Leave_applications_model->get_one($applicaiton_id);
        $appr=array();
        if($applicatoin_info->last_approved){
        $appr[]=$applicatoin_info->last_approved;
        }
        $appr[]=$this->login_user->id;
        $app=implode(",",$appr);
        ...
        $md=0;$sm=0;$gm=0;$hr=0;
        if ($this->login_user->is_admin) {    
            $leave_data['is_md_approved']=$sts; 
            $leave_data['md_approved_time']= get_current_utc_time();   
            $md=1;       
        }else if($this->login_user->role_id==1){
            $leave_data['is_gm_approved']=$sts;   
            $leave_data['gm_approved_time']= get_current_utc_time();        
            $gm=1; 
            
        }else if($this->login_user->role_id==2){
            $leave_data['is_sm_approved']=$sts;
            $leave_data['sm_approved_time']= get_current_utc_time();
            $sm=1;   
            
        }else if($this->login_user->role_id==4){
            $leave_data['is_hr_approved']=$sts;
            $leave_data['hr_approved_time']= get_current_utc_time();   
            $hr=1;
            
        }
//darini 22-2
        $role=$this->Users_model->get_one( $applicatoin_info->applicant_id);
        if($role->role_id==3){
            if(($applicatoin_info->is_md_approved==1 || $md==1 )&& ($applicatoin_info->is_gm_approved==1 || $gm==1 ) && ($applicatoin_info->is_sm_approved==1|| $sm==1) && ($applicatoin_info->is_hr_approved==1 || $hr==1) ){
                $leave_data["status"]=  "approved";
            }else{
                $leave_data["status"]=  "pending";
            }
        }else if($role->role_id==1){
            if(($applicatoin_info->is_md_approved==1 || $md==1 )){
                $leave_data["status"]=  "approved";
            }else{
                $leave_data["status"]=  "pending";
            }
        }else if($role->role_id==2){
            if(($applicatoin_info->is_md_approved==1 || $md==1 )&& ($applicatoin_info->is_gm_approved==1 || $gm==1 )  && ($applicatoin_info->is_hr_approved==1 || $hr==1) ){
                $leave_data["status"]=  "approved";
            }else{
                $leave_data["status"]=  "pending";
            }
        }else if($role->role_id==4){
            if(($applicatoin_info->is_md_approved==1 || $md==1 )&& ($applicatoin_info->is_gm_approved==1 || $gm==1 )  ){
                $leave_data["status"]=  "approved";
            }else{
                $leave_data["status"]=  "pending";
            }
