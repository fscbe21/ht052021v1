...
class Logistics_members extends MY_Controller {
    ...
    public function index() {
        ...
        $this->template->rander("team_members/logistics_index", $view_data);
    }

    /* open new member modal */

    function modal_form() {
       // $this->access_only_admin();  //ANAND10022021
        ...
        $options = array(
		 ...
		 //R.V_01_03S
        $view_data['id_proof'] = $this->Id_proof_model->get_details()->result();
        $view_data['address_proof'] = $this->Address_proof_model->get_details()->result();
        $view_data['department_list'] = $this->Department_model->get_details()->result();
//R.V_01_03E

        $view_data['model_info'] = $this->Logistic_users_model->get_details($options)->row();
    ...
    function modal_form_target($id) {
       ...
       //  $id = $this->input->post('userid');
         ...
         $this->load->view('team_members/modal_form_target', $view_data);
     }
    ...
    function add_team_member() {
       // $this->access_only_admin(); //ANAND10022021
        ...
        if ($this->Logistic_users_model->is_email_exists($this->input->post('email'))) {
        ...
        $files_data = move_files_from_temp_dir_to_permanent_dir($target_path, "user_id_proof");
        //R.V01_03E
        ...
        $user_data = array(
            ...
            "gender" => $this->input->post('gender'),
            "user_type" => "staff",
            "last_job_title" => $this->input->post('last_job_title'),
            "qualification" => $this->input->post('qualification'),
            "status" => 'inactive',
            //r.v start
            "emp_id" => $this->input->post('emp_id'),//hc
            "alternative_phone" => $this->input->post('alternative_phone'),

            "web" => $this->input->post('web'),//hc
            "fathers_name" => $this->input->post('fathers_name'),//hc
            "mothers_name" => $this->input->post('mothers_name'),//hc
            "spouse_name" => $this->input->post('spouse_name'),//hc
            "academic_quali" => $this->input->post('academic_quali'),//hc
            "experience" => $this->input->post('experience'),//hc
            "id_name" => $this->input->post('id_name'),//hc
            "id_num" => $this->input->post('id_num'),//hc
            "department" => $this->input->post('department'),//hc
            "reference" => $this->input->post('reference'),//hc
            "marital_status" => $this->input->post('marital_status'),//hc
            "dob" => $this->input->post('dob'),//hc

            //R.v End22_2
			
			//R.V01_03S
            "bank_acc_no" => $this->input->post('bank_acc_no'),//hc
            "account_type" => $this->input->post('account_type'),//hc
            "acc_holder_name" => $this->input->post('acc_holder_name'),//hc
            "ifsc_code" => $this->input->post('ifsc_code'),//hc
            "bank_name" => $this->input->post('bank_name'),//hc
            "branch_name" => $this->input->post('branch_name'),
            "id_proof" => $this->input->post('id_proof'),//hc
            "addr_proof" => $this->input->post('addr_proof'),
//R.V01_03E
        ...
        $user_data["id_files"] = $files_data;
//R.V01_03Ey


        //add a new team member
        $user_id = $this->Logistic_users_model->save($user_data);
        if ($user_id) {
            ...
            $this->Logistic_users_model->save_job_info($job_data);
    ...
    function list_data() {
        ...
        $list_data = $this->Logistic_users_model->get_details($options)->result();
        $result = array();
    ...
    function _row_data($id) {
        ...
        $data = $this->Logistic_users_model->get_details($options)->row();
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $show_cotact_info = 1; //$this->can_view_team_members_contact_info();//ANAND10022021

        $row_data = array(
            $user_avatar,
            get_team_member_profile_link($data->id, $full_name),
            /* modal_anchor(get_uri("team_members/modal_form_target/".$data->id), $full_name, array("class" => "clickuserid", "title" => "Sales Target", "data-userid" => $data->id)), */
       ...
       /*  if ($this->login_user->is_admin && $this->login_user->id != $data->id) {
        ...
        if($data->status == 'active'){
            /* $delete_link = anchor(get_uri("team_members/decline/" . $data->id), "<i class='fa fa-times'></i>", array("title" => 'Decline')); */

            $delete_link = 
            modal_anchor(get_uri("team_members/modal_form_target/".$data->id), '<i class="fa fa-bullseye"></i>', array("class" => "clickuserid", "title" => "Sales Target", "data-userid" => $data->id))
            .anchor(get_uri("team_members/decline/" . $data->id), "<i class='fa fa-times'></i>", array("title" => 'Decline'));
        }else{
            $delete_link = "Inactive ".anchor(get_uri("team_members/approve/" . $data->id), "<i class='fa fa-check'></i>", array("title" => 'Approve'));
        }
    ...
    function delete() {
       // $this->access_only_admin();
        ...
        if ($id != $this->login_user->id && $this->Logistic_users_model->delete($id)) {
    ...
    function approve($id) {
        $this->Logistic_users_model->approve_user($id);
        redirect("team_members");
 
     }

     function decline($id) {
       $this->Logistic_users_model->decline_user($id);
       redirect("team_members");
    ...
    function view($id = 0, $tab = "") {
        if ($id * 1) {
            ...
            $user_info = $this->Logistic_users_model->get_details($options)->row();
            if ($user_info) {
            ...
            $view_data['team_members'] = $this->Logistic_users_model->get_details(array("user_type" => "staff", "status" => "active"))->result();
    ...
    function job_info($user_id) {
        ...
        $view_data['job_info'] = $this->Logistic_users_model->get_job_info($user_id);
        $view_data['department_info'] = $this->Department_model->get_details()->result();
    ...
    function save_job_info() {
        ...
        $this->Logistic_users_model->save($user_data, $user_id);
        if ($this->Logistic_users_model->save_job_info($job_data)) {
    ...
    function general_info($user_id) {
        ...
        $view_data['user_info'] = $this->Logistic_users_model->get_one($user_id);
        $view_data['department_info'] = $this->Department_model->get_details()->result();
    ...
    function save_general_info($user_id) {
        ...
        $user_info_updated = $this->Logistic_users_model->save($user_data, $user_id);
    ...
    function my_preferences() {
        $view_data["user_info"] = $this->Logistic_users_model->get_one($this->login_user->id);
    ...
    function save_my_preferences() {
        ...
        $this->Logistic_users_model->save($user_data, $this->login_user->id);
    ...
    function save_account_settings($user_id) {
        ...
        if ($this->Logistic_users_model->is_email_exists($this->input->post('email'), $user_id)) {
            echo json_encode(array("success" => false, 'message' => lang('duplicate_email')));
            exit();
        }
        $account_data = array(
        ...
        if ($this->Logistic_users_model->save($account_data, $user_id)) {
            echo json_encode(array("success" => true, 'message' => lang('record_updated')));
        } else {
            echo json_encode(array("success" => false, 'message' => lang('error_occurred')));
        }
    ...
    function save_profile_image($user_id = 0) {
        $this->update_only_allowed_members($user_id);
        $user_info = $this->Logistic_users_model->get_one($user_id);
        ...
        if ($profile_image) {
            ...
            $this->Logistic_users_model->save($image_data, $user_id);
    ...
    function expense_info($user_id) {
        ...
        $view_data['expenses'] = $this->Expenses_model->get_details($options)->result();
    ...
    function get_user_details($user_id){
        ...
        $user_info =  $this->Logistic_users_model->get_details(array("id" => $user_id))->row();
  
         echo json_encode($user_info);
  
      }
      //end start 3feb2021 dsk
      ...
      function add_target(){
        $target_data = array(
            ...
            "actual_days" => $this->input->post('actual_days'),
            "target_amount" => $this->input->post('target'),
            "average" => $this->input->post('avg'),
            "created_user_id" => $this->login_user->id,
            "user_id" => $this->input->post('empid')
        );

        $target_data = clean_data($target_data);

        $save_target_data = $this->Target_model->save($target_data);

        /* if ($save_target_data) {
            echo json_encode(array("success" => true, 'message' => 'Data submitted successfully'));
        } else {
            echo json_encode(array("success" => false, 'message' => 'Please try again later !'));
        } */
